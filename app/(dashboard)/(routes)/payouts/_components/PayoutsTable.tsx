...
export function PayoutTable() {
  ...
  const totalItems = payoutsData.length;

  // Filter and sort data
  const filteredData = payoutsData.filter((item) => {
    const matchesSearch =
      item.merchant.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.sourceAccount.includes(searchTerm) ||
      item.destinationAccount.includes(searchTerm);
    const matchesDate =
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    switch (filter.sortBy) {
      ...
      case "status-successful-first":
        return a.status === "Successful" ? -1 : b.status === "Successful" ? 1 : 0;
      case "status-failed-first":
        return a.status === "Failed" ? -1 : b.status === "Failed" ? 1 : 0;
      case "status-processing-first":
                ...
                <div className="flex flex-col space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="status-successful-first">Status (Successful First)</SelectItem>
                      <SelectItem value="status-failed-first">Status (Failed First)</SelectItem>
                      <SelectItem value="status-processing-first">Status (Processing First)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex flex-col space-y-2">
          ...
          <TableBody>
            {paginatedData.map((item) => (
              <TableRow key={item.sN}>
                ...
                <TableCell>{item.paymentRef}</TableCell>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      <DropdownMenuItem onClick={() => setSelectedPayout(item)}><View /> View</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => console.log("Download", item.sN)}><Download /> Download</DropdownMenuItem>
                      {item.status === "Failed" && (
                        <DropdownMenuItem onClick={() => console.log("Retry", item.sN)}><Retry/> Retry</DropdownMenuItem>
                      )}
                    </DropdownMenuContent>
                  </DropdownMenu>
                </TableCell>
